...
#include <math.h>
#include <lo/lo.h>
#include <time.h>  // Added for clock_gettime and struct timespec

#ifndef _WIN32
#include <termios.h>  // Only for Linux
#endif

#ifdef _WIN32
#include <conio.h>  // For _kbhit and _getch on Windows
#define INTERFACE_NAME "\\\\Device\\\\NPF_{YOUR_WINDOWS_NIC_GUID}"  // Replace with actual Windows NIC (e.g., from Npcap/WinPcap)
#else
#define INTERFACE_NAME "enp0s31f6"  // Linux
#endif
...
#else
// No-op on Windows (handled by _kbhit/_getch)
void setup_keyboard() {}
void restore_keyboard() {}
#endif

// Cross-platform non-blocking key read (-1 if no key pressed)
int get_key()
{
#ifndef _WIN32
    return getchar();  // Returns -1 (EOF) if no input (due to non-blocking)
#else
    if (_kbhit()) {
        return _getch();
    }
